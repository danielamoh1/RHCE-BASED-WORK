...
I created five RHEL 8 virtual machines to be able to successfully complete all the tasks.

One VM will be configured as an Ansible control node. Other four VMs will be used to apply playbooks to solve the tasks. The following FQDNs will be used throughout this project.
...
>  `/etc/hosts` created and hosts file is already being populated with ip addresses and hostnames along with FQDNs.

> just an example of the `/etc/hosts` file
...
> Next i will connect to control node via ssh, the examiner would tell you the user that you will connect as to your control node with `matthew`
...
> copy your public key to managed nodes. As root generating ssh key and copy it to the managed hosts:
...
Generate an SSH keypair on the control node.
...
> this helps you in setting up ansible lab. this is regarding configuring ssh key and public key and copy the key to managed nodes.
...
> make sure you use quotation around your description parameters.
...
## Group differentiation "File Content"
...
## Group differentiation "File Content" complete
...
> Another way of completing this task:
...
> It's always good to know how to add facts to ansible_facts.

## Ansible Facts complete
...
## Work with Ansible Facts complete
...
## Text Manipulation also called "Configure SSH Server"
...
## Text Manipulation also called "Configure SSH Server" complete
...
## Use Conditionals to Control Play Execution "Conditionals"
...
## Use Conditionals to Control Play Execution "Conditionals" complete
...
## Software Repositories complete
...
> You need to ssh hosts in database, and then use this command
...
> sometimes templating doesn't work for various reasons, my answer could be also misleading, you should check ansible documentations to make sure you are using jinja loop structure correctly and also the way you access hostvars[host] data structure correctly.
...
In my home lab, i wrote a playbook that creates account for new employees. The idea is to execute the playbook each time a new person joins the company. To ease the boarding process your playbook should ask the user for his username and password while executing. All the people that are going to execute the playbook are suppossed to be part of `networking team`. From time to time they will need to interact with nmcli tool but despite that they shouldn't have access to `privileged commands`.
...
## Create and Work with Roles (apache role)
...
## Create and Work with Roles (apache role) complete
...
## Download Roles From Ansible Galaxy and Use Them complete
...
## Copy file and modify it's content with the use of ansible facts
...
## Copy file and modify it's content with the use of ansible facts complete
...
## timesynce role using rhel-system-roles
...
## timesynce role using rhel-system-roles complete
...
> second, I take a look at the directory structure for roles, once rhel system roles is installed i'll be able traverse this directory below, and look for files, each file is full of explanation of how these roles work, i strongly advise you to take a look at it prior to writing your playbook
...
## Selinux and fcontext playbook complete

> In selinux, when you create a file and you want this file to be displayed as part of httpd service this file need to have the same selinux context as `/var/www/html/` files.
...
> Another solution to the problem, this solution might be more accurate than mine.
...
## Use Ansible Galaxy to install a role
...
## Use Ansible Galaxy to install a role complete
...
## Use RHEL System Roles complete
...
## Using roles from Ansible Galaxy complete

> you need to make sure to  install other roles via `requirement.yml` which would install `phpinfo` and `balancers` roles.
...
## Create and use partitions complete
